...
class IManageLocalGraphs(form.Schema):  # Interface):
    ...
    graph = schema.List(title=_(u"Graphs"),
        value_type=schema.Choice(
            title=_(u"Graph"),
            vocabulary="gu.plone.rdf.LocalGraphVocabulary",
            )
        )
...
class ManageLocalGraphsForm(form.SchemaForm):
    ...
    ignoreContext = True
    template = ViewPageTemplateFile('managelocalgraphs.pt')

    schema = IManageLocalGraphs
    ...
    def handleUpload(self, action):
        ...
        if not error:
            try:
                ...
                g = self.getGraph(data)
                rdftool = getUtility(IORDF)
                # FIXME: local store not part of contract
